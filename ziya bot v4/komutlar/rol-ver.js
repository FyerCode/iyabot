...
exports.run = async (client, message, args) => {
      ...
      if (!message.member.hasPermission("MANAGE_ROLES")) return message.channel.send(new Emran.MessageEmbed().setDescription('Bu komudu kullanmak için **Rolleri Yönet** yetkisine sahip olmalısın.').setColor('YELLOW'));
...
await (member.roles.add(role.id))
 message.channel.send(new Emran.MessageEmbed().setDescription(`${member} isimli üyeye \`${role.name}\` isimli yetki başarıyla verildi!`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('YELLOW'));
