...
exports.run = async (client, message, args) => {
    const ayarlar = require('../ayarlar.json')
        ...
        if (!message.member.permissions.has("MANAGE_ROLES")) return message.channel.send(new emran.MessageEmbed().setDescription('Bu komudu kullanmak için **Rolleri Yönet** yetkisine sahip olmalısın.').setColor('YELLOW'));
...
if (!role) return message.channel.send('Rol bulunamadı.');
    if (!member.roles.cache.has(role.id)) return message.channel.send(new emran.MessageEmbed().setDescription('Kullanıcı O Yetkiye Sahip Değil.').setColor('YELLOW'));
...
await (member.roles.remove(role.id))
 message.channel.send(new emran.MessageEmbed().setDescription(`${member} isimli üyenin \`${role.name}\` isimli yetkisi başarıyla alındı!`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('YELLOW'));
